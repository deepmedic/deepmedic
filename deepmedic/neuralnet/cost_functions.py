...
def x_entr( p_y_given_x_train, y_gt, weightPerClass, eps=1e-6 ):
    ...
    weightPerClass5D = tf.reshape(weightPerClass, shape=[1, tf.shape(input=p_y_given_x_train)[1], 1, 1, 1])
    weighted_log_p_y_given_x_train = log_p_y_given_x_train * weightPerClass5D
    
    y_one_hot = tf.one_hot( indices=y_gt, depth=tf.shape(input=p_y_given_x_train)[1], axis=1, dtype="float32" )
    
    num_samples = tf.cast( tf.reduce_prod( input_tensor=tf.shape(input=y_gt) ), "float32")
