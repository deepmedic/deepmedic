...
class Pathway(object):
    ...
    def apply(self, input, mode, train_val_test, verbose=False, log=None):
        ...
        if verbose:
            log.print3(
                "Pathway [" + str(self.getStringType()) + "], Mode: [" + mode + "], Input's Shape: " + str(input.shape))
        ...
        for idx, block in enumerate(self._blocks):
            if verbose:
                log.print3("\tBlock [" + str(idx) + "], Mode: [" + mode + "], Input's Shape: " + str(
                    input_to_next_layer.shape))
            ...
            if idx not in self._inds_of_blocks_for_res_conns_at_out:  # not a residual connecting here
                ...
                log.print3(
                    "\tResidual-Connection made between: output of [Block_" + str(idx) + "] & input of previous block.")
                out_res = ops.make_residual_connection(input_to_prev_layer, out)
                input_to_prev_layer = input_to_next_layer
                input_to_next_layer = out_res

        if verbose:
            log.print3("Pathway [" + str(self.getStringType()) + "], Mode: [" + mode + "], Output's Shape: " + str(
                input_to_next_layer.shape))
