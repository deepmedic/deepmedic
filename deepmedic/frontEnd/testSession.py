...
class TestSession(Session):
    ...
    def make_output_folders(self):
        ...
        [self._out_folder_logs, self._out_folder_preds, self._out_folder_fms] = make_folders_for_test_session(
    ...
    def _ask_user_if_test_with_random(self):
        ...
        try:
            user_input = input(
                "WARN:\t Testing was requested, but without specifying a pretrained, saved model to load.\n"
    ...
    def run_session(self, sess_device, model_config: ModelConfig):
        ...
        with tf.compat.v1.Session(
            ...
            res_code = inference_on_whole_volumes(
                *([sessionTf, cnn3d] + self._params.get_args_for_testing() + [inp_shapes_per_path])
            )
