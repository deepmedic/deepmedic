...
class TestSessionParameters(object):
    ...
    def errorIntNormZScoreTwoAppliesGiven():
        print(
            "ERROR: In testing-config, for the variable (dictionary) norm_zscore_prms,"
    ...
    def __init__(self, log, main_outp_folder, out_folder_preds, out_folder_fms, n_classes, cfg):
        ...
        if cfg[cfg.DATAFRAME] is not None:  # get data from csv/dataframe
            ...
            (self.channels_fpaths, self.gt_fpaths, self.roi_fpaths, self.out_preds_fnames) = get_paths_from_df(
                self.log, self.dataframe, os.path.dirname(self.csv_fname), req_gt=False
            )
            ...
            self.gt_fpaths = (
                parse_filelist(abs_from_rel_path(cfg[cfg.GT_LBLS], abs_path_cfg), make_abs=True)
                if cfg[cfg.GT_LBLS] is not None
                else None
            )
            self.roi_fpaths = (
                parse_filelist(abs_from_rel_path(cfg[cfg.ROIS], abs_path_cfg), make_abs=True)
                if cfg[cfg.ROIS] is not None
                else None
            )
            self.out_preds_fnames = (
                parse_filelist(abs_from_rel_path(cfg[cfg.FNAMES_PREDS], abs_path_cfg))
                if cfg[cfg.FNAMES_PREDS] is not None
                else None
            )

        # predictions
        self.save_segms = cfg[cfg.SAVE_SEGM] if cfg[cfg.SAVE_SEGM] is not None else True
        self.save_probs_per_cl = (
            cfg[cfg.SAVE_PROBMAPS_PER_CLASS]
        ...
        self.suffixes_for_outp = (
            cfg[cfg.SUFFIX_SEGM_PROB]
            if cfg[cfg.SUFFIX_SEGM_PROB] is not None
        ...
        if self.save_fms_flag:
            inds_fms = (
                [cfg[cfg.INDICES_OF_FMS_TO_SAVE_NORMAL]]
    ...
    def print_params(self):
        ...
        if not (self.save_segms or self.save_probs_per_cl):
            logPrint(
                ">>> WARN: Segmentation and Probability Maps won't be saved. I guess you only wanted the feature maps?"
            )

        logPrint("~~~~~~~Ouput-parameters for Feature Maps (FMs)~~~~~~")
        logPrint("Save FMs in individual images = " + str(self.save_fms_flag))
        logPrint(
            "Indices of min/max FMs to save, per type of pathway (normal/subsampled/FC) and per layer = "
        ...
        if not self.pad_input:
            logPrint(
                ">>> WARN: Inference near the borders of the image might be incomplete if not padded!"
