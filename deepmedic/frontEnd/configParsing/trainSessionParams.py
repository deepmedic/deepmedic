...
from deepmedic.frontEnd.configParsing.utils import (
    abs_from_rel_path,
    parse_filelist,
    check_and_adjust_path_to_ckpt,
    get_paths_from_df,
    parse_fpaths_of_channs_from_filelists,
)
...
class TrainSessionParameters(object):
    ...
    def errorIntNormZScoreTwoAppliesGiven():
        print(
            "ERROR: In training-config, for the variable (dictionary) norm_zscore_prms,"
    ...
    def __init__(
        self, log, main_outp_folder, out_folder_models, out_folder_preds, out_folder_fms, n_classes, model_name, cfg
    ):
        ...
        if cfg[cfg.DATAFRAME_TR] is not None:
            ...
            (self.channels_fpaths_tr, self.gt_fpaths_tr, self.roi_fpaths_tr, _) = get_paths_from_df(
                self.log, self.dataframe_tr, os.path.dirname(self.csv_fname_train), req_gt=True
            )
        else:
            ...
            self.roi_fpaths_tr = (
                parse_filelist(abs_from_rel_path(cfg[cfg.ROIS_TR], abs_path_cfg), make_abs=True)
                if cfg[cfg.ROIS_TR] is not None
                else None
            )
        ...
        if sampling_type_flag_tr in [0, 3] and cfg[cfg.PROP_OF_SAMPLES_PER_CAT_TR] is not None:
            ...
            self.sampling_type_inst_tr.set_perc_of_samples_per_cat([1.0 / n_sampl_cats_tr] * n_sampl_cats_tr)
        ...
        self.lr_sched_params = {
            ...
            "predef": {
                "epochs": cfg[cfg.PREDEF_SCH],
                "div_lr_by": cfg[cfg.DIV_LR_BY] if cfg[cfg.DIV_LR_BY] is not None else 2.0,
            },
            "auto": {
                "min_incr_of_val_acc_considered": cfg[cfg.AUTO_MIN_INCR_VAL_ACC]
                if cfg[cfg.AUTO_MIN_INCR_VAL_ACC] is not None
                else 0.0,
                "epochs_wait_before_decr": cfg[cfg.NUM_EPOCHS_WAIT] if cfg[cfg.NUM_EPOCHS_WAIT] is not None else 5,
                "div_lr_by": cfg[cfg.DIV_LR_BY] if cfg[cfg.DIV_LR_BY] is not None else 2.0,
            },
            "poly": {
                "epochs_wait_before_decr": cfg[cfg.NUM_EPOCHS_WAIT]
                if cfg[cfg.NUM_EPOCHS_WAIT] is not None
                else self.n_epochs / 3,
        ...
        if (not self.val_on_samples_during_train) and (not self.val_on_whole_volumes):
            ...
            ) = get_paths_from_df(self.log, self.dataframe_val, os.path.dirname(self.csv_fname_val), req_gt=True)
            ...
            self.gt_fpaths_val = (
                parse_filelist(abs_from_rel_path(cfg[cfg.GT_LBLS_VAL], abs_path_cfg), make_abs=True)
                if cfg[cfg.GT_LBLS_VAL] is not None
                else self.errorReqGtLabelsVal()
            )
            self.roi_fpaths_val = (
                parse_filelist(abs_from_rel_path(cfg[cfg.ROIS_VAL], abs_path_cfg), make_abs=True)
                if cfg[cfg.ROIS_VAL] is not None
                else None
            )
            self.out_preds_fnames_val = (
                parse_filelist(abs_from_rel_path(cfg[cfg.FNAMES_PREDS_VAL], abs_path_cfg))
                if cfg[cfg.FNAMES_PREDS_VAL]
                else None
            )
        ...
        if (
            not self.out_preds_fnames_val
            and self.val_on_whole_volumes
            and (self.save_segms_val or True in self.save_probs_per_cl_val or self.save_fms_flag_val)
        ):
        ...
        self.norm_prms = {
            "verbose_lvl": cfg[cfg.NORM_VERB_LVL] if cfg[cfg.NORM_VERB_LVL] is not None else 0,
        ...
        layers_to_freeze_per_pathtype = [
            cfg[cfg.LAYERS_TO_FREEZE_NORM],
        ...
        self.losses_and_weights = (
            cfg[cfg.LOSSES_WEIGHTS] if cfg[cfg.LOSSES_WEIGHTS] is not None else {"xentr": 1.0, "iou": None, "dsc": None}
        )
    ...
    def _backwards_compat_with_deprecated_cfg(self, cfg):
        ...
        if cfg[cfg.REFL_AUGM_PER_AXIS] is not None:
            self.augm_sample_prms_tr["reflect"] = [0.5 if bool else 0.0 for bool in cfg[cfg.REFL_AUGM_PER_AXIS]]
    ...
    def print_params(self):
        ...
        logPrint(
            "Type of Sampling = "
            + str(self.sampling_type_inst_tr.get_type_as_str())
            + " ("
        ...
        logPrint(
            "Percent of Samples to extract per Sampling Category = "
            + str(self.sampling_type_inst_tr.get_perc_to_sample_per_cat())
        )
        logPrint(
            "Paths to weight-Maps for sampling of each category = "
        ...
        logPrint(
            "Number of cases to load per Subepoch (for extracting the samples for this subepoch) = "
            + str(self.max_n_cases_per_subep_train)
        )
        logPrint(
            "Number of Segments loaded per subepoch for Training = "
            + str(self.n_samples_per_subep_train)
        ...
        logPrint(
            "[Predef] Predefined schedule of epochs when the LR will be lowered = "
            + str(self.lr_sched_params["predef"]["epochs"])
        )
        logPrint(
            "[Predef] When decreasing Learning Rate, divide LR by = " + str(self.lr_sched_params["predef"]["div_lr_by"])
        )
        logPrint(
            "[Poly] Initial epochs to wait before lowering LR = "
        ...
        logPrint(
            "[Auto] Initial epochs to wait before lowering LR = "
        ...
        logPrint(
            "[Auto] Minimum increase in validation accuracy (0. to 1.) that resets the waiting counter = "
        ...
        logPrint(
            "Type of Sampling = "
            + str(self.sampling_type_inst_val.get_type_as_str())
            + " ("
        ...
        logPrint(
            "Perform Full-Inference on Val. cases every that many epochs = "
        ...
        logPrint("Suffixes with which to save segmentations and probability maps = " + str(self.suffixes_for_outp_val))
        logPrint("~~Feature Maps~~")
        logPrint("Save Feature Maps = " + str(self.save_fms_flag_val))
        logPrint(
            "Min/Max Indices of FMs to visualise per pathway-type and per layer = "
