    ...
    def override_file_cfg_with_cmd_line_cfg(self, log, args):
        if args.saved_model:
            ...
            if self.get(self.SAVED_MODEL) is not None:
                log.print3(
                    "WARN: A model to load was specified both in command line and in the test-config file!\n"
                    + "\t The input by the command line will be used: "
                    + str(abs_path_model_cmd_line)
                )
