...
class TestConfig(Config):
    ...
    CHANNELS = config_data.add_elem("channels", required=True, description="Input Channels",
    ...
    SAVE_SEGM = config_data.add_elem("saveSegmentation", elem_type='Bool', default=True,
                                     description="Save Segmentation",
                                     info="Specify whether to save segmentation maps.")
    SAVE_PROBMAPS_PER_CLASS = config_data.add_elem("saveProbMapsForEachClass",
                                                   description="Save per Class Probability Maps",
                                                   info="Provide a list with as many entries as there are classes. "
                                                        "True/False to save/not the probability map for the "
                                                        "corresponding class.\n"
                                                        "(default: [True,True...for all classes])")
    SUFFIX_SEGM_PROB = config_data.add_elem("suffixForSegmAndProbsDict",
    ...
    INDICES_OF_FMS_TO_SAVE_NORMAL = config_data.add_elem("minMaxIndicesOfFmsToSaveFromEachLayerOfNormalPathway",
                                                         description="Indices of FMs to Save per Layer (Normal)",
                                                         info="A model may have too many feature maps, and some may "
                                                              "not be needed. For this, we allow specifying which FMs "
                                                              "to save.\n"
                                                         ...
                                                         default=[])
    INDICES_OF_FMS_TO_SAVE_SUBSAMPLED = config_data.add_elem("minMaxIndicesOfFmsToSaveFromEachLayerOfSubsampledPathway",
                                                             description="Indices of FMs to Save per Layer "
                                                                         "(Subsampled)",
                                                             info="A model may have too many feature maps, and some "
                                                                  "may not be needed. For this, we allow specifying "
                                                                  "which FMs to save.\n"
                                                                  ...
                                                                  "save from the layer. See FC for example.",
                                                             default=[])
    INDICES_OF_FMS_TO_SAVE_FC = config_data.add_elem("minMaxIndicesOfFmsToSaveFromEachLayerOfFullyConnectedPathway",
    ...
    PAD_INPUT = config_data.add_elem("padInputImagesBool", elem_type='Bool', description="Pad Input Images",
