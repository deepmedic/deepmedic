...
def abs_from_rel_path(pathGiven, absolutePathToWhereRelativePathRelatesTo):
    ...
    if os.path.isabs(pathGiven):
        ...
        if os.path.isdir(absolutePathToWhereRelativePathRelatesTo):
            ...
            print(
                "ERROR: in [func:returnAbsolutePathEvenIfRelativePathIsGiven()] Given path :",
                absolutePathToWhereRelativePathRelatesTo,
                " does not correspond to neither an existing file nor a directory. Exiting!",
            )
            exit(1)
...
def checkIfAllElementsOfAListAreFilesAndExitIfNot(pathToTheListingFile, list1):
    for filepath in list1:
        if not os.path.isfile(filepath):
            print(
                "ERROR: in [checkIfAllElementsOfAListExistAndExitIfNot()] path:",
                filepath,
                " given in :",
                pathToTheListingFile,
                " does not correspond to a file. Exiting!",
            )
            exit(1)
...
def checkListContainsCorrectNumberOfCasesOtherwiseExitWithError(
    ...
    if numberOfCasesPreviously != numberOfContainedCasesInList:
        raise IOError(
            "ERROR: Given file:",
            pathToGivenListFile + "\n\t contains #",
            numberOfContainedCasesInList,
            " entries, whereas previously checked files contained #",
            numberOfCasesPreviously,
            "."
...
def checkThatAllEntriesOfAListFollowNameConventions(listOfPredictionNamesForEachCaseInListingFile):
    for entry in listOfPredictionNamesForEachCaseInListingFile:
        if entry.find("/") > -1 or entry.startswith("."):
            raise IOError(
                "ERROR: Check that all entries follow name-conventions failed." + '\n\t Entry "',
                entry,
                "\" was found to begin with '.' or contain '/'. Please correct this.",
            )
...
def check_and_adjust_path_to_ckpt(log, filepath_to_ckpt):
    ...
    if index_of_str > -1 and len(filepath_to_ckpt) > len(filepath_to_ckpt[:index_of_str]) + len(STR_DM_CKPT):  # found.
        ...
        user_input = None
        string_warn = (
            "It seems that the path to the model to load paramters from, a tensorflow checkpoint, was given wrong."
            ...
            + "\n\t Given path seemed longer: "
            + str(filepath_to_ckpt)
        )
        try:
            user_input = input(
                ">>\t "
