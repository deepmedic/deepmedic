...
def inferenceWholeVolumes(sessionTf,
    ...
    log.print3(
        "###########################################################################################################")
    log.print3("############################# Starting full Segmentation of " + str(
    ...
    for image_i in range(total_number_of_images):
        ...
        for batch_i in range(num_batches):
            ...
            if saveIndividualFmImagesForVisualisation or saveMultidimensionalImageWithAllFms:
                ...
                # currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray is the index in the
                # multidimensional array that holds all the to-be-visualised-fms. It is the one that corresponds to
                # the next to-be-visualised indexOfTheLayerInTheReturnedListByTheBatchTraining.
                currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray = 0
                # indexOfTheLayerInTheReturnedListByTheBatchTraining is the index over all the layers in the
                ...
                for pathway in cnn3d.pathways:
                    for layer_i in range(len(pathway.getLayers())):
                        ...
                        # defines were to put them in the multidimensional-image-array.
                        highIndexOfFmsInTheMultidimensionalImageToFillInThisIterationExcluding = \
                            currentIndexInTheMultidimensionalImageWithAllToBeVisualisedFmsArray + \
                            indicesOfFmsToExtractFromThisLayer[1] - \
                            indicesOfFmsToExtractFromThisLayer[0]
                        ...
                        numberOfVoxToSubtrToGetPatchWidthAtThisFm_R = \
                            numberOfCentralVoxelsClassified[0] - 1 if pathway.pType() != pt.SUBS else \
                                int(math.ceil((numberOfCentralVoxelsClassified[0] * 1.0) / pathway.subsFactor()[0]) - 1)

                        numberOfVoxToSubtrToGetPatchWidthAtThisFm_C = \
                            numberOfCentralVoxelsClassified[1] - 1 if pathway.pType() != pt.SUBS else \
                                int(math.ceil((numberOfCentralVoxelsClassified[1] * 1.0) / pathway.subsFactor()[1]) - 1)

                        numberOfVoxToSubtrToGetPatchWidthAtThisFm_Z = \
                            numberOfCentralVoxelsClassified[2] - 1 if pathway.pType() != pt.SUBS else \
                        ...
                        rPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions = \
                            fmsReturnedForATestBatchForCertainLayer.shape[2] \
                            - numberOfVoxToSubtrToGetPatchWidthAtThisFm_R

                        cPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions = \
                            fmsReturnedForATestBatchForCertainLayer.shape[3] \
                            - numberOfVoxToSubtrToGetPatchWidthAtThisFm_C
                        zPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions = \
                            fmsReturnedForATestBatchForCertainLayer.shape[4]\
                            - numberOfVoxToSubtrToGetPatchWidthAtThisFm_Z
                        ...
                        rOfTopLeftCentralVoxelAtTheFm = \
                            (rPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions - 1) // 2
                        cOfTopLeftCentralVoxelAtTheFm = \
                            (cPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions - 1) // 2
                        zOfTopLeftCentralVoxelAtTheFm = \
                            (zPatchDimensionAtTheFmThatWeVisualiseAfterConvolutions - 1) // 2
                        ...
                        numberOfCentralVoxelsToGetInDirectionR = int(math.ceil(
                            (numberOfCentralVoxelsClassified[0] * 1.0) / pathway.subsFactor()[
                                0])) if pathway.pType() == pt.SUBS else numberOfCentralVoxelsClassified[0]
                        numberOfCentralVoxelsToGetInDirectionC = int(math.ceil(
                            (numberOfCentralVoxelsClassified[1] * 1.0) / pathway.subsFactor()[
                                1])) if pathway.pType() == pt.SUBS else numberOfCentralVoxelsClassified[1]
                        numberOfCentralVoxelsToGetInDirectionZ = int(math.ceil(
                            (numberOfCentralVoxelsClassified[2] * 1.0) / pathway.subsFactor()[
                                2])) if pathway.pType() == pt.SUBS else numberOfCentralVoxelsClassified[2]
                        # ============================================================================================

                        # Grab the central voxels of the predicted fms from the cnn in this batch.
                        centralVoxelsOfAllFmsInLayer = \
                            fmsReturnedForATestBatchForCertainLayer[:,  # batchsize
                        ...
                        if pathway.pType() == pt.SUBS:
                            expandedOutputOfFmsR = np.repeat(centralVoxelsOfAllFmsInLayer, pathway.subsFactor()[0],
                                                             axis=2)
                        ...
                        for imagePart_in_this_batch_i in range(batchsize):
                            ...
                            sliceCoordsOfThisSegment = sliceCoordsOfSegmentsInImage[
                                imagePartOfConstructedFeatureMaps_i + imagePart_in_this_batch_i]
                            ...
                            fmImageInMultidimArrayToReconstructInThisIteration[:,  # last dimension is the
                                                                                   # number-of-Fms, I create an image
                                                                                   # for each.
                ...
                imagePartOfConstructedFeatureMaps_i += batchsize
